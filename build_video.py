...
def make_thumbnail(image_path, text, dest, dim=0.45):
    """1280x720 thumbnail: first image darkened, with big white text and a black outline."""
    ...
    while size > 40:
        ...
        if max(widths) + 2 * stroke <= tw - 140 and line_h * len(lines) <= th - 100:
            break
        size -= 6
...
def upload_to_youtube(path, payload, thumbnail=None):
    ...
    if thumbnail and pathlib.Path(thumbnail).exists():
        try:
            ...
            print("WARNING: thumbnail upload failed:", e, flush=True)
    return response["id"]
